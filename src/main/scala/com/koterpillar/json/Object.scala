...
sealed trait Object[A] extends Schema[A] {
  ...
  override def codec: Prism[Json, A] = Object.jsonObject.composePrism(objectCodec)
}

object Object {
  def jsonObject: Prism[Json, JsonObject] = Prism((j: Json) => j.asObject)(Json.fromJsonObject)
}

case object NullObject extends Object[HNil] {
  override def fields: List[Field_] = Nil

  override def objectCodec: Prism[JsonObject, HNil] = Prism[JsonObject, HNil] {
    obj =>
    if(obj.toMap == Map.empty) Some(HNil) else None
  }(_ => JsonObject.empty)
}

final case class ConsObject[H, T <: HList](head: Field[H], tail: Object[T]) extends Object[H :: T] {
  override def fields: List[Field_] = head :: tail.fields

  override def objectCodec: Prism[JsonObject, H :: T] = {
    def getOption(json: JsonObject): Option[H :: T] = for {
